...
const checkTicketFilter = (ticket) => {
  ...
  if (DirectTicketsToWallets() && (ticket?.contact?.wallets?.length || 0) > 0) {
    ...
    if (idx !== -1) {
      console.log('Ticket de la billetera del usuario')
      return true
    }
    console.log('DirectTicketsToWallets: El ticket no pertenece a la billetera del usuario', ticket)
    return false
  }
  ...
  if (isNotViewAssignedTickets() && (ticket?.userId || userId) !== userId) {
    console.log('isNotViewAssignedTickets y el ticket no pertenece al usuario', ticket?.userId, userId)
  ...
  if (filtros.isNotAssignedUser) {
    console.log('isNotAssignedUser Habilitado para mostrar solo tickets sin firmar', filtros.isNotAssignedUser, !ticket.userId)
