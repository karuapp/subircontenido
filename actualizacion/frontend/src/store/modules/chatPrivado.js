    ...
    LIST_USERS_PRIVATE(state, payload) {
      if (payload.action === 'create') {
        ...
        if (index >= 0) {
          ...
          state.listaUsuarios.push(payload.data)
        }
        state.listaUsuarios.sort((a, b) => new Date(Number(b.timestamp)) - new Date(Number(a.timestamp)))
    ...
    LIST_GROUPS_PRIVATE_CHAT(state, payload) {
      if (payload.action === 'create') {
        ...
        if (index >= 0) {
          ...
          state.listaGrupos.push(payload.data)
        }
        state.listaGrupos.sort((a, b) => new Date(Number(b.timestamp)) - new Date(Number(a.timestamp)))
      }
    },
