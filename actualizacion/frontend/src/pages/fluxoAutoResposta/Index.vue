...
export default {
  ...
  data () {
    return {
      ...
      columnsEtapas: [
        ...
        { name: 'initialStep', label: 'Paso inicial', sortable: true, field: 'initialStep', align: 'left', format: v => v ? 'Sim' : '' },
        { name: 'acoes', label: '', field: 'acciones', align: 'center' }
      ],
      columnsAcoes: [
        ...
        { name: 'userIdDestination', label: 'Usuario de destino', field: 'userIdDestination', align: 'center', format: (v) => v ? this.usuarios.find(u => u.id === v).name : '' },
        { name: 'nextStepId', label: 'ID del paso de destino', field: 'nextStepId', align: 'center' },
  ...
  methods: {
    ...
    deletarEtapaAutoResposta (autoResposta, etapa) {
      ...
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `¿Realmente deseas eliminar la etapa "ID: ${etapa.id}"?`,
        cancel: {
          label: this.$t('common.no'),
          color: 'primary',
          push: true
        ...
        DeletarEtapaResposta(dataParams)
          .then(res => {
            ...
            this.$q.notify({
              ...
              progress: true,
              position: 'top',
              message: `Etapa ${etapa.id} Eliminado!`,
              actions: [{
                icon: 'close',
                round: true,
                color: 'white'
    ...
    deletarAcaoEtapa (autoReply, etapa, acao) {
      ...
      this.$q.dialog({
        ...
        DeletarAcaoEtapa(acao)
          ...
          .catch(error => {
            console.error(error)
            this.$notificarErro('No es posible eliminar la acción de etapa', error)
          })
        this.loading = false
      })
    },
