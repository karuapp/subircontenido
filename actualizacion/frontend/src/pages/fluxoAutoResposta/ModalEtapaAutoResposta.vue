<template>
  ...
  >
    ...
    >
      <q-card-section>
        <div class="text-h6">{{ etapaAutoRespostaEdicao.id ? 'Editar Etapa' : 'Nueva Etapa' }}</div>
      </q-card-section>
      <q-card-section>
        <q-input
      ...
      try {
        ...
        if (etapa.id) {
          ...
          this.$q.notify({
            ...
            position: 'top',
            textColor: 'black',
            message: '¡Paso editado exitosamente!',
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
        ...
        console.error(error)
        this.$notificarErro('Se produjo un error al guardar el paso.', error)
      }
    }
  },
  watch: {
