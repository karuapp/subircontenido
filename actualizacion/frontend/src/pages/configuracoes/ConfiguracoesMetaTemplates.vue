  ...
  <div v-if="userProfile === 'admin'">
    ...
    <q-dialog v-model="showTestModal" persistent>
      <q-card style="min-width: 400px">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">Enviar plantilla de prueba</div>
          <q-space />
          <q-btn icon="close" flat round dense v-close-popup />
        </q-card-section>
  ...
  methods: {
    ...
    async criarTemplate() {
      try {
        ...
        if (error.response?.data?.error === 'Formato de nombre no válido') {
          this.$q.notify({
            type: 'warning',
            message: this.$t('configuracaoMetaPanel.notifications.nameFormatted'),
            timeout: 8000
          })
        } else if (error.response?.data?.error === 'Límite de caracteres excedido' || error.response?.data?.error === 'Límite de caracteres excedido') {
    ...
    async editarTemplateSubmit() {
      try {
        ...
        if (error.response?.data?.error === 'La plantilla de ejemplo no se puede editar') {
          this.$q.notify({
            type: 'warning',
            message: this.$t('configuracaoMetaPanel.notifications.sampleTemplateEdit'),
            timeout: 5000
          })
        } else if (error.response?.data?.error === 'Formato de nombre no válido') {
          this.$q.notify({
            type: 'warning',
            message: this.$t('configuracaoMetaPanel.notifications.nameFormatted'),
            timeout: 8000
    ...
    async confirmarExclusao(template) {
      this.$q.dialog({
        ...
        try {
          ...
          // Verificar se é um erro específico sobre templates de exemplo
          if (error.response?.data?.error === 'La plantilla de ejemplo no se puede eliminar') {
            this.$q.notify({
              type: 'warning',
    ...
    adicionarComponente() {
      ...
      if (novoComponente.type === 'BUTTONS') {
        this.$nextTick(() => {
          novoComponente.buttons = []
        })
      }
      
      this.novoTipoComponente = null // Limpar o seletor após adicionar
      
      // Atualizar preview após adicionar componente
      this.atualizarPreview()
    ...
    removerComponente(index) {
      ...
      // Atualizar preview após remover componente
      this.atualizarPreview()
    },
    ...
    adicionarBotao(componente) {
      ...
      // Atualizar preview após adicionar botão
      this.atualizarPreview()
    },
    ...
    removerBotao(componente, index) {
      ...
      // Atualizar preview após remover botão
      this.atualizarPreview()
    },
    ...
    onComponentTypeChange(componente) {
      ...
      // Atualizar preview após alterar tipo do componente
      this.atualizarPreview()
    },
    ...
    async enviarTemplateTeste() {
      try {
        ...
        console.error('Error al enviar la plantilla de prueba:', error)
        
        // Verificar se é um erro específico sobre formato do número
        if (error.response?.data?.error === 'Formato de número no válido') {
          this.$q.notify({
            type: 'warning',
    ...
    async registrarTelefone() {
      try {
        ...
        console.error('Error al registrar el número de teléfono:', error)
    ...
    async verificarCodigo() {
      try {
        ...
        console.error('Error al verificar el código. No se encontró el metatoken. Configure el token en la sección de configuración.:', error)
    ...
    async abrirModalWebhook() {
      ...
      try {
        ...
        if (!metaToken) {
          this.$q.notify({
            type: 'negative',
            message: 'Error al verificar el código. No se encontró el metatoken. Configure el token en la sección de configuración.',
            timeout: 5000
          })
          return
        }
        ...
        console.error('Error al buscar metaToken:', error)
        this.$q.notify({
          type: 'negative',
          message: 'Error al recuperar la configuración de la empresa. Inténtalo de nuevo.',
          timeout: 5000
        })
      }
    },
    ...
    async sobrescreverWebhook() {
      try {
        ...
        console.error('Error al sobrescribir el webhook:', error)
        console.error('Detalles del error:', {
          message: error.message,
          response: error.response?.data,
          status: error.response?.status
        ...
        this.$q.notify({
          type: 'negative',
          message: error.response?.data?.error || 'Error al configurar el webhook. Revisa los detalles y vuelve a intentarlo.',
          timeout: 5000
        })
      } finally {
        this.loadingWebhook = false
    ...
    async configurarSIP() {
      try {
        ...
        console.error('Error al configurar SIP:', error)
