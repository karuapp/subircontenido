        ...
        <q-toolbar class="bg-grey-3 text-black row">
            <q-btn v-if="$q.screen.width > 500" flat dense icon="mdi-emoticon-happy-outline" :disable="cDisableActions || cMostrarEnvioArquivo" class="bg-padrao btn-rounded q-mx-xs" :color="$q.dark.isActive ? 'white' : ''">
              ...
              <q-menu anchor="top right" self="bottom middle" :offset="[5, 40]">
                <VEmojiPicker style="width: 40vw" :showSearch="false" :emojisByRow="20" :labelSearch="$t('common.search')" lang="es" @select="onInsertSelectEmoji" />
              </q-menu>
            </q-btn>
            ...
            <q-input hide-bottom-space :disable="cDisableActions" v-show="!cMostrarEnvioArquivo" ref="inputEnvioMensagem" id="inputEnvioMensagem" type="textarea" @keydown.exact.enter.prevent="() => newMenssage.trim().length ? sendMessage() : ''" class="col-grow q-mx-xs text-grey-10 inputEnvioMensagem" bg-color="white" color="grey-7" placeholder="Escribe tu mensaje" input-style="max-height: 30vh" autogrow rounded dense outlined v-model="newMenssage" @paste="handlePaste">
            ...
            <q-file
              ...
              accept=".mp3, .txt, .xml, .jpg, .png, .gif, image/gif, image/jpeg, .pdf, .doc, .docx, .mp4, .xls, .xlsx, .jpeg, .zip, .ppt, .pptx, image/*"
  ...
  methods: {
    ...
    async handleSartRecordingAudio() {
      try {
        if (!navigator?.mediaDevices?.getUserMedia) {
          throw new Error('getUserMedia No compatible con este navegador')
        }
        ...
        if (typeof window === 'undefined' || typeof window.MediaRecorder === 'undefined') {
          throw new Error('MediaRecorder No compatible con este navegador')
        }
        this.audioChunks = []
        ...
        mr.onstop = async () => {
          try {
            ...
            console.error('No se pudo finalizar la grabación:', err)
    ...
    abrirEnvioArquivo(event) {
      ...
      try {
        ...
        console.error('Error al abrir el selector de archivos', e)
      }
    },
    onNativeFileChange(e) {
      try {
        ...
        for (const f of files) {
          if (f.size > MAX_EACH) {
            this.$q.notify({ type: 'negative', message: `${f.name} arriba 15MB`, position: 'top' })
          } else {
            validBySize.push(f)
          }
        }
    ...
    async marcarMensagemComoLida(contactId, isGroup) {
      ...
      try {
        ...
        console.error('Error al marcar el mensaje como no leído:', error)
      }
    },
    async sendMessage() {
