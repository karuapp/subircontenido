<template>
  <q-page class="q-pa-md" v-if="userProfile === 'superadmin'">
    ...
    >
      ...
      <q-tab name="encryptionkey" :label="$t('customizar.tabs.encryptionkey')" />
      <!-- <q-tab name="proxies" :label="$t('configuracoes.sidebar.proxy')" /> -->
    </q-tabs>
    ...
    <q-tab-panels v-model="tab" animated>
      ...
      <!-- Tab de Encryption Key -->
      <q-tab-panel name="encryptionkey">
        <q-card flat bordered class="my-card">
          <q-card-section>
            <div class="text-h6 q-mb-md">{{ $t('customizar.encryptionkey.title') }}</div>
            <div class="text-caption q-mb-lg">
              {{ $t('customizar.encryptionkey.description') }}
            </div>
          </q-card-section>
          <q-card-section>
            <div class="row q-col-gutter-md">
              <div class="col-12 col-md-6">
                <q-input
                  v-model="encryptionkeyValue"
                  :label="$t('customizar.encryptionkey.keyLabel')"
                  :type="showEncryptionkey ? 'text' : 'password'"
                  filled
                  dense
                  @update:model-value="updateEncryptionkeyConfig"
                >
                  <template v-slot:prepend>
                    <q-icon name="lock" />
                  </template>
                  <template v-slot:append>
                    <q-btn
                      :icon="showEncryptionkey ? 'visibility_off' : 'visibility'"
                      flat
                      dense
                      @click="showEncryptionkey = !showEncryptionkey"
                    />
                  </template>
                </q-input>
                <div class="q-mt-sm">
                  <q-btn
                    :label="$t('customizar.encryptionkey.generateRandom')"
                    color="primary"
                    outline
                    dense
                    @click="generateRandomEncryptionkey"
                    class="full-width"
                  >
                    <template v-slot:prepend>
                      <q-icon name="refresh" />
              ...
              <div class="col-12 col-md-6">
                <q-card flat bordered>
                  <q-card-section>
                    <div class="text-subtitle2 q-mb-sm">{{ $t('customizar.encryptionkey.infoTitle') }}</div>
                    <div class="text-caption">
                      {{ $t('customizar.encryptionkey.infoText') }}
                    </div>
                  </q-card-section>
                </q-card>
              </div>
            ...
            <div class="q-mt-md">
              ...
              >
                ...
                <div class="text-weight-medium">
                  {{ $t('customizar.encryptionkey.warningTitle') }}
                </div>
                <div class="text-caption">
                  {{ $t('customizar.encryptionkey.warningText') }}
                </div>
              </q-banner>
            </div>
          </q-card-section>
...
import { AlterarTenantPostmanLink, ListarTenants, AlterarTenantForcarLogout, AlterarTenantMasterKey, AlterarTenantEncryptionKey } from 'src/service/tenants.js';
import { 
  ListarProxies, 
  SalvarProxies, 
  RemoverProxies, 
...
export default {
  ...
  computed: {
    ...
    colorDefs () {
      ...
      return {
        neutral: this.$t('customizar.colors.neutral') || 'Color neutro',
        primary: this.$t('customizar.colors.primary') || 'Color primario',
        secondary: this.$t('customizar.colors.secondary') || 'Color secundario',
        accent: this.$t('customizar.colors.accent') || 'Color de acento',
        warning: this.$t('customizar.colors.warning') || 'Color de advertencia',
        negative: this.$t('customizar.colors.negative') || 'Color negativo',
        positive: this.$t('customizar.colors.positive') || 'Color positivo',
        light: this.$t('customizar.colors.light') || 'Color claro'
      }
    }
  },
  data() {
    return {
      ...
      proxyCount: 5,
      proxyType: { label: 'SOCKS', value: 'socks' },
      proxyTypes: [
  ...
  created() {
    ...
    this.loadEncryptionkeyConfig();
    this.carregarProxies();
  ...
  methods: {
    ...
    async uploadLogoDarkFile() {
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          console.log(this.$t('customizar.notifications.successUploadLogo') || 'Logotipo oscuro enviado correctamente');
        } else {
          console.error(this.$t('customizar.notifications.errorUploadLogo') || 'Error al enviar el logotipo oscuro');
        }
      } catch (error) {
        console.error(this.$t('customizar.notifications.errorUploadLogo') || 'Error al enviar el logotipo oscuro', error);
      }
    },
    ...
    async updateForceLogoutConfig() {
      ...
      try {     
        ...
        if (response.status === 200) {
          ...
          this.$q.notify({
            type: 'negative',
            message: 'Error al actualizar la configuración de cierre de sesión forzado',
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          if (masterTenant) {
            ...
            this.postmanLink = masterTenant.postmanLink || '';
          }
        } else {
    ...
    async updateMasterkeyConfig() {
      try {
        ...
        if (response.status === 200) {
          this.$q.notify({
            type: 'positive',
            message: 'Configuración de la llave maestra actualizada correctamente',
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
          ...
          this.$q.notify({
            type: 'negative',
            message: 'Error al actualizar la configuración de la llave maestra',
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
    ...
    async loadEncryptionkeyConfig() {
      try {
        ...
        if (response.status === 200) {
          const masterTenant = response.data.find(t => t.id === 1);
          if (masterTenant) {
            this.encryptionkeyValue = masterTenant.encryptionkey || '';
          }
        } else {
          console.error('Erro ao carregar a configuração da encryptionkey');
        }
      } catch (error) {
        console.error('Erro ao carregar a configuração da encryptionkey:', error);
      }
    },
    async updateEncryptionkeyConfig() {
      try {
        const requestData = {
          id: 1, // Sempre o tenant 1
          encryptionkey: this.encryptionkeyValue
        };
        
        const response = await AlterarTenantEncryptionKey(requestData);
        
        if (response.status === 200) {
          this.$q.notify({
            type: 'positive',
            message: this.$t('customizar.encryptionkey.successUpdate'),
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
          ...
          this.$q.notify({
            type: 'negative',
            message: this.$t('customizar.encryptionkey.errorUpdate'),
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
        ...
        console.error('Error updating encryption key configuration:', error);
        this.$q.notify({
          type: 'negative',
          message: this.$t('customizar.encryptionkey.errorUpdate'),
          position: 'top',
    ...
    generateRandomEncryptionkey() {
      const length = 64; // Tamanho maior para chave de criptografia
      const characters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=';
      let randomString = '';
      for (let i = 0; i < length; i++) {
        const randomIndex = Math.floor(Math.random() * characters.length);
      ...
      this.encryptionkeyValue = randomString;
      this.updateEncryptionkeyConfig(); // Salva a chave gerada
    },

    // Métodos relacionados a proxies
    ...
    async gerarProxies() {
      try {
        ...
        console.error('Erro ao gerar proxies:', error)
        this.$q.notify({
          type: 'negative',
    ...
    async gerarProxiesGlobais() {
      try {
        ...
        console.error('Erro ao gerar proxies globais:', error)
        this.$q.notify({
          type: 'negative',
    ...
    async removerProxies() {
      try {
        ...
        console.error('Erro ao remover proxies:', error)
        this.$q.notify({
          type: 'negative',
    ...
    async salvarEdicao() {
      try {
        ...
        console.error('Erro ao editar proxy:', error)
        this.$q.notify({
          type: 'negative',
