    ...
    <q-card class="q-pa-md"
      ...
      <q-card-section>
          ...
          <div class="q-mt-md row full-width justify-center"
            ...
            <div class="col">
              <fieldset class="full-width q-pa-md">
                <legend>Datos de tu cuenta de Instagram</legend>
                <div class="col-12 q-mb-md"
                  v-if="whatsapp.type === 'instagram'">
                  <c-input outlined
                    :label="$t('sessaoModalWhatsapp.user')"
                    v-model="whatsapp.instagramUser"
                    hint="Tu nombre de usuario de Instagram (sin @)" />
                </div>
        ...
        <!-- <div class="row q-my-md">
          <div class="col-12">
            <label class="text-caption">Mensaje de despedida de Atención al Cliente:</label>
  ...
  data () {
    return {
      ...
      variaveis: [
        ...
        { label: 'Asistente (si está asistiendo)', value: '{{user}}' },
        { label: 'E-mail del agente (si está agente)', value: '{{userEmail}}' },
    ...
    async handleSaveWhatsApp (whatsapp) {
      ...
      try {
        ...
        if (error.data.error === 'ERR_NO_PERMISSION_CONNECTIONS_LIMIT') {
          Notify.create({
            type: 'negative',
            message: 'Se alcanzó el límite de conexión.',
          ...
          return this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: '¡Uy! Revisar errores... El nombre de la conexión no puede existir en la plataforma; es un identificador único.',
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
