<template>
  <q-dialog v-model="modalOpen" persistent ref="buscarMensagensDialog">
    <q-card style="width: 500px; max-width: 90vw;">
      ...
      <q-card-actions align="right" class="q-gutter-sm">
        ...
        />
        <q-btn flat v-close-popup :label="$t('buscarMensagemModal.closeButton')" color="negative" @click="closeModal" class="text-uppercase" />
      </q-card-actions>
    </q-card>
...
export default {
  ...
  computed: {
    ...
    uniqueTickets() {
      ...
      const seen = new Set();
      const source = Array.isArray(this.tickets) ? this.tickets : [];
      return source
        .filter(t => t && t.ticket && t.ticket.whatsappId)
        .filter(ticketObj => {
          const lastTs = ticketObj.lastMessageTimestamp || '';
          const key = `${ticketObj.ticket.id}-${lastTs}`;
          if (seen.has(key)) {
            return false;
          }
          seen.add(key);
  ...
  methods: {
    ...
    closeModal() {
      ...
      try {
        this.$refs.buscarMensagensDialog && this.$refs.buscarMensagensDialog.hide && this.$refs.buscarMensagensDialog.hide()
      } catch (e) {}
