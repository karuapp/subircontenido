...
export default {
  ...
  methods: {
    ...
    formatarMensagemWhatsapp (body) {
      ...
      // Aplicar formatação de links diretamente aqui para evitar conflitos com v-linkified
      const urlRegex = /(https?:\/\/[^\s]+)/g
      format = format.replace(urlRegex, (url) => `<a href="${url}" target="_blank" rel="noopener">${url}</a>`)
          ...
          if (component.type === 'HEADER') {
              ...
              component.buttons.forEach(button => {
                  ...
                  if (button.type === 'URL') {
                      ...
                      buttonsHtml += `<a href="tel:${button.phone_number || '#'}" style="display: inline-block; margin: 5px; padding: 10px; background-color: #28a745; color: white; text-decoration: none; border-radius: 5px;">📞 ${buttonText}</a>`;
                      ...
                      buttonsHtml += `<button style="display: inline-block; margin: 5px; padding: 10px; background-color: #25d366; color: white; border: none; border-radius: 5px;" title="Catálogo de WhatsApp">📋 ${buttonText}</button>`;
                  } else if (button.type === 'button' && button.sub_type === 'catalog') {
                      buttonsHtml += `<button style="display: inline-block; margin: 5px; padding: 10px; background-color: #25d366; color: white; border: none; border-radius: 5px;" title="Catálogo de WhatsApp">📋 Ver catálogo</button>`;
                  } else if (button.type === 'button' && button.sub_type === 'flow') {
                      buttonsHtml += `<button style="display: inline-block; margin: 5px; padding: 10px; background-color: #ff6b35; color: white; border: none; border-radius: 5px;" title="Flujo interactivo">🔄 Iniciar flujo</button>`;
                  } else if (button.type === 'button' && button.sub_type === 'FLOW') {
                    buttonsHtml += `<button style="display: inline-block; margin: 5px; padding: 10px; background-color: #ff6b35; color: white; border: none; border-radius: 5px;" title="Flujo interactivo">🔄 Iniciar flujo</button>`;
                }
              });
    ...
    formatarBotaoWhatsapp(body) {
      ...
      try {
        ...
        console.error('Botón de error de formato WhatsApp:', error)
        return body
      }
    },
    formatarBotao(body) {
      ...
      try {
        ...
        function whatsappStyles(format, wildcard, opTag, clTag) {
          ...
          try {
            ...
            console.error('Error al aplicar estilos WhatsApp:', error)
          }
          return format
        }
        ...
        console.error('Mensaje de error de formato de lista:', error)
        return body
      }
    }, 
      ...
      try {
        ...
        if (infoMidia) {
          ...
          linhas.forEach(linha => {
            ...
            if (linha.startsWith('📱 Medios:')) {
              const tipo = linha.replace('📱 Medios:', '').trim();
              html += `<div class="midia-card">
                        <div class="midia-icon">📱</div>
                        <div class="midia-info">
                          <div class="midia-label">Tipo de medio</div>
              ...
              html += `<div class="midia-card">
                        <div class="midia-icon">🔗</div>
                        <div class="midia-info">
                          <div class="midia-label">Enlace de medios</div>
              ...
              const descricao = linha.replace('📝 Descrição:', '').trim();
              html += `<div class="midia-card">
                        <div class="midia-icon">📝</div>
                        <div class="midia-info">
        ...
        html += `<div class="banner-anuncio" style="background: #f0f0f0; border-radius: 8px; padding: 4px 12px; margin: 4px 0 8px 0; max-width: 85%; text-align: center;">
                  <span style="color: #666; font-size: 11px; font-weight: 500;">Enviado por anuncio</span>
                </div>`;
      ...
      try {
        ...
        if (infoAnuncio) {
          ...
          linhas.forEach(linha => {
            ...
            // Link da mídia
            else if (linha.startsWith('[Multimedia]')) {
              linkMidia = linha.replace('[Multimedia]', '').trim();
            }
            // Link do thumbnail
          ...
          if (descricaoAnuncio) {
            html += `<div class="midia-card" style="display: flex; align-items: center; padding: 10px; margin: 8px 0; background: linear-gradient(135deg, #4ecdc4, #44a08d); border-radius: 8px; color: white;">
                      <div class="midia-icon" style="width: 32px; height: 32px; background: rgba(255,255,255,0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-weight: bold; font-size: 14px;">📝</div>
                      <div class="midia-info" style="flex: 1;">
                        <div class="midia-label" style="font-size: 11px; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.5px;">Descrição</div>
    ...
    formatarAdsMessage(body) {
      ...
      try {
        ...
        // Extrair [Mídia]
        const midiaMatch = body.match(/\[Mídia\]\s*(https?:\/\/[^\n]+)/i);
        if (midiaMatch) {
        ...
        // Extrair [Conversão]
        const conversaoMatch = body.match(/\[Conversão\][^\n]*/i);
        if (conversaoMatch) {
        ...
        if (midiaUrl) {
          html += `<div style="margin-top: 8px;">
                    <a href="${midiaUrl}" target="_blank" style="color: #056162; text-decoration: underline;">Ver el video</a>
                  </div>`;
        }
        ...
        console.error('Erro ao formatar mensagem de anúncio:', error);
        return this.formatarMensagemWhatsapp(body);
      }
    },
    formatarOrder(body) {
    ...
    formatarOrderMessage(body) {
      try {
        ...
        html += '</div>';
        html += '<div>';
        html += '<div style="font-weight: bold; font-size: 18px; color: #495057; margin-bottom: 4px; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">Pedido por WhatsApp</div>';
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Detalles del pedido</div>';
        html += '</div>';
        html += '</div>';
    ...
    formatarProductMessage(body) {
      try {
        ...
        html += '</div>';
        html += '<div>';
        html += '<div style="font-weight: bold; font-size: 18px; color: #495057; margin-bottom: 4px; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">Producto</div>';
        ...
        if (temImagem) {
          html += `<div style="text-align: center; margin: 20px 0;">
                    <img src="${imagemBase64}" alt="Produto" style="max-width: 200px; max-height: 150px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); object-fit: cover;">
                  </div>`;
        }
        ...
        if (preco) {
          html += `<div style="text-align: center; margin: 20px 0;">
                    <div style="background: linear-gradient(135deg, #e53e3e, #c53030); color: white; border-radius: 12px; padding: 15px; display: inline-block; box-shadow: 0 4px 8px rgba(229,62,62,0.3);">
                      <div style="font-size: 12px; opacity: 0.8; margin-bottom: 5px;">Preço</div>
    ...
    formatarLocationRequest(body) {
      try {
        ...
        html += '</div>';
        html += '<div>';
        html += '<div style="font-weight: bold; font-size: 18px; color: #495057; margin-bottom: 4px; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">Solicitud de ubicación</div>';
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Haga clic en el botón para compartir</div>';
        html += '</div>';
        html += '</div>';
        ...
        console.error('Erro ao formatar solicitação de localização:', error);
        // Fallback para formatação simples
    ...
    formatarCTAURL(body) {
      try {
        ...
        html += '</div>';
        html += '<div>';
        html += '<div style="font-weight: bold; font-size: 18px; color: #495057; margin-bottom: 4px; text-shadow: 0 1px 2px rgba(0,0,0,0.1);">URL de llamada a la acción</div>';
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Clique no botão para acessar o link</div>';
        html += '</div>';
        html += '</div>';
