<template>
  <q-list separator
    ...
    <q-item clickable
      ...
      @click="() => { abrirChatContato(ticket); marcarComoLido(ticket) }"
      ...
      <q-item-section id="ListItemsTicket" >
        <q-item-label class="text-bold blur-effect"
          ...
          <template v-if="usuarioDados?.restrictedUser === 'enabled'">
            {{ ((!ticket.name ? ticket?.contact?.name || 'Sin nombre' : ticket?.name || 'Sin nombre').substring(0, 5)) }}{{ ((!ticket.name ? ticket?.contact?.name || 'Sin nombre' : ticket?.name || 'Sin nombre').length > 5 ? '...' : '') }}
          </template>
          <template v-else>
            {{ !ticket.name ? ticket?.contact?.name || 'Sin nombre' : ticket?.name || 'Sin nombre' }}
          </template>
        ...
        <q-btn flat
          ...
          dense
          round
          v-if="(!$q.screen.xs && (ticket.status === 'pending' || (buscaTicket && ticket.status === 'pending'))) && controlFeatures"
          class="q-mr-md">
          ...
          <q-dialog v-model="isTicketModalOpen">
            <q-card :style="cardStyle">
              <q-card-section class="row items-center justify-between">
                ...
                <!-- <div class="text-h6">{{ 'Espiar chat: ' + currentTicket.id}}</div> -->
                <q-btn icon="close" flat round @click="closeModal" />
              </q-card-section>
              <q-card-section>
                <MensagemChat :mensagens="currentTicket.messages" />
        ...
        <q-btn flat
          ...
          dense
          round
          v-if="(!$q.screen.xs && (ticket.status === 'open' || (buscaTicket && ticket.status === 'open'))) && controlFeatures"
          class="q-mr-md">
          <q-avatar>
            <q-icon size="20px"
...
import { AtualizarTicketNaoLido } from 'src/service/tickets.js'
