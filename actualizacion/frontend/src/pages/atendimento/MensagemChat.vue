          ...
          <div style="min-width: 100px; max-width: 350px" :style="mensagem.isDeleted ? 'color: rgba(0, 0, 0, 0.36) !important;' : ''">
              ...
              <q-menu square auto-close anchor="bottom left" self="top left">
                <q-list style="min-width: 100px">
                  <q-item :disable="!['whatsapp', 'telegram', 'baileys', 'evo', 'meow', 'waba', 'uazapi', 'zapi'].includes(ticketFocado.channel)" clickable @click="citarMensagem(mensagem)"  v-if="(ticketFocado.channel === 'whatsapp' || ticketFocado.channel === 'baileys' || ticketFocado.channel === 'evo' || ticketFocado.channel === 'meow' || ticketFocado.channel === 'waba' || ticketFocado.channel === 'uazapi' || ticketFocado.channel === 'zapi') && (mensagem.mediaType !== 'notes' && mensagem.mediaType !== 'transcription' && mensagem.mediaType !== 'transfer') ">
                    ...
                    <q-tooltip v-if="!['whatsapp', 'telegram', 'baileys'].includes(ticketFocado.channel)"> Disponible solo para WhatsApp, Telegram y WhatsApp Business QR. </q-tooltip>
                  </q-item>
                  ...
                  <q-item :disable="!['whatsapp', 'telegram', 'baileys', 'waba', 'meow', 'evo', 'uazapi', 'zapi'].includes(ticketFocado.channel)" clickable @click="mensagemReacao = mensagem; modalEmojiOpen = true" v-if="(mensagem.mediaType !== 'notes' && mensagem.mediaType !== 'transcription') && (ticketFocado.channel === 'whatsapp' || ticketFocado.channel === 'baileys' || ticketFocado.channel === 'waba' || ticketFocado.channel === 'meow' || ticketFocado.channel === 'evo' || ticketFocado.channel === 'uazapi' || ticketFocado.channel === 'zapi')">
                  ...
                  <q-item :disable="!['whatsapp', 'telegram', 'baileys', 'waba', 'evo', 'meow', 'uazapi', 'zapi'].includes(ticketFocado.channel)" clickable @click="baixarAudio(mensagem)" v-if="(mensagem.mediaType === 'audio' || mensagem.mediaType === 'audioMessage')">
            ...
            <div class="contact-container">
              <q-avatar size="30px" class="q-mr-sm">
                <img
                  v-if="!mensagem.fromMe && mensagem.contact?.profilePicUrl"
                  class="blur-effect"
            ...
            <div
              ...
              v-if="!['vcard', 'contactMessage', 'contactsArrayMessage','application', 'documentMessage', 'audio', 'button', 'reply_buttons', 'list', 'location', 'locationMessage', 'liveLocationMessage', 'location_request', 'cta_url', 'interactive', 'interactive_nfm_reply', 'button_reply', 'sticker', 'notes', 'templates', 'transcription', 'transfer', 'templateMessage', 'adsMessage', 'productMessage', 'order', 'orderMessage'].includes(mensagem.mediaType)"
    ...
    <q-dialog v-model="modalEmojiOpen" :maximized="$q.screen.lt.sm" :position="$q.screen.gt.xs ? 'standard' : 'bottom'">
      <q-card class="emoji-picker-modal" :style="$q.screen.gt.xs ? 'min-width: 500px; max-width: 600px; max-height: 85vh;' : 'width: 100%; max-height: 85vh;'">
        <q-card-section class="q-pa-none" style="overflow: hidden;">
          <VEmojiPicker
            :style="$q.screen.gt.xs ? 'width: 100%; height: 75vh;' : 'width: 100%; height: 70vh;'"
            :showSearch="true"
  ...
  methods: {
    ...
    async downloadFile(fileUrl, fileName) {
      try {
        ...
        this.$q.notify({
          color: 'positive',
          position: 'top',
          message: this.$t('common.downloadSuccess') || '¡La descarga comenzó exitosamente!',
        });
      } catch (error) {
    ...
    formatarMensagemCTAURL(body) {
      try {
        ...
        console.error('Error al formatear la URL del mensaje de CTA:', error);
    ...
    calculateEmojisByRow() {
      ...
      // Para o modal de emoji, calcular baseado na largura disponível
      // O modal tem largura de 500-600px em telas maiores
      if (screenWidth < 600) {
        return 8; // Aumentado para telas pequenas
      } else if (screenWidth >= 600 && screenWidth < 1200) {
        return 12; // Aumentado para telas médias
      } else {
        return 15; // Ajustado para caber no modal sem scroll horizontal
      }
    },
...
.emoji-picker-modal {
  ...
  :deep(.v3-emoji-picker) {
    ...
    // Garantir que não haja scroll horizontal em nenhum elemento
    * {
      max-width: 100% !important;
      box-sizing: border-box !important;
    }
