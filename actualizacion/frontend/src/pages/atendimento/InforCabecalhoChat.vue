...
export default {
  ...
  computed: {
    ...
    cSessions() {
      return this.whatsapps.filter(w => ["whatsapp", "baileys", "meow", "waba", "evo", "uazapi", "zapi"].includes(w.type) && !w.isDeleted && w.status === 'CONNECTED');
    },
    cSessionsOptions() {
  ...
  methods: {
    ...
    async buscarMensagensConversa() {
      ...
      try {
        ...
        console.error('Error al cargar el historial de búsqueda:', e);
    ...
    irParaMensagem(mensagem) {
      this.$emit('buscar:scrollToMessage', mensagem.id);
      this.modalBuscarMensagem = false;
      this.$nextTick(() => {
        const selector = `[data-message-id="${mensagem.id}"]`;
        const elemento = document.querySelector(selector);
        if (elemento) {
          try { elemento.scrollIntoView({ behavior: 'smooth', block: 'center' }); } catch(e) {}
    ...
    async confirmarGrupoUsuarios() {
      try {
        ...
        console.error('Error al actualizar usuarios del grupo:', error);
        this.$q.notify({
          type: 'negative',
  ...
  mounted(){
    ...
    try {
      ...
      console.error('Error al cargar datos de usuario localStorage:', error)
    }
  }
}
</script>
