  ...
  methods: {
    ...
    addNewLineCondition (from, to, oldTo) {
      ...
      if (oldTo) {
        ...
        // Remove apenas a conexão visual específica
        try {
                ...
                if (conn && conn.source && conn.target) {
                  try {
                    ...
                    if (!connError.message?.includes("Cannot read properties of null")) {
                      console.error('Error al eliminar una conexión específica:', connError)
                    }
                  }
                }
              })
      ...
      // REMOVIDO: this.clickNode(from) para evitar abrir o painel automaticamente
    },
    saveFlow () {
      const data = {
        ...this.cDataFlow.flow,
    ...
    deleteLine (from, to) {
      ...
      if (isProtected) {
        ...
        this.$q.notify({
          ...
          position: 'top',
          timeout: 2500,
          message: `Esta conexión entre "${fromNode?.name || 'Inicio'}" y "${toNode?.name || 'Configuración'}" Esto es obligatorio y no se puede eliminar.`,
          actions: [{
            icon: 'close',
            round: true,
            color: 'white'
      ...
      // Remove a conexão visual do jsPlumb com tratamento de erro
      if (from && to) {
        try {
          ...
          if (conn && conn.source && conn.target) {
            // Usa setTimeout para evitar problemas de timing do jsPlumb
            setTimeout(() => {
              try {
                this.jsPlumb.deleteConnection(conn)
              } catch (connError) {
                // Silencia o erro específico do jsPlumb que não afeta a funcionalidade
                if (!connError.message?.includes("No se pueden leer las propiedades de null")) {
    ...
    deleteNode (node) {
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `¿Realmente desea eliminar el elemento (${node.name})?`,
        cancel: {
          label: this.$t('common.no'),
          color: 'primary',
          push: true
        ...
        if (this.data.lineList) {
          ...
          // Remove linhas onde o nó é origem
          this.data.lineList = this.data.lineList.filter(line => {
        ...
        // Remove todas as conexões visuais do jsPlumb conectadas a este nó
        try {
          // Remove conexões onde o nó é origem
          const sourceConnections = this.jsPlumb.getConnections({
            source: node.id
          })
          
          // Remove conexões onde o nó é destino
          ...
          console.error('Error al obtener conexiones para la eliminación del nodo:', error)
        }
    ...
    syncLinesWithConditions() {
      ...
      // Adiciona conexões protegidas às linhas esperadas
      expectedLines.push(...protectedConnections)
      ...
      // Adiciona linhas que estão faltando
    ...
    downloadData () {
      this.$q.dialog({
        ...
        this.$notificarSucesso('Descargando, por favor espere....')
      })
    },
    openHelp () {
