...
export default {
  ...
  methods: {
      ...
      if (this.$refs[audioRef]) {
        try {
          ...
          this.$refs[audioRef].play().catch(error => {
            console.error(`Error al reproducir el sonido DTMF ${number}:`, error)
          })
        } catch (error) {
          console.error(`Error al reproducir el sonido DTMF ${number}:`, error)
        }
    ...
    playCallingTone() {
      if (this.$refs.callingTone) {
        try {
          ...
          console.error('Error al reproducir el sonido de llamada:', error)
        }
      }
    },
    ...
    stopCallingTone() {
      if (this.$refs.callingTone) {
        try {
          ...
          console.error('Error al detener el sonido de la llamada:', error)
        }
      }
    },
    ...
    async testWebSocket() {
      return new Promise((resolve, reject) => {
        console.log('Intentando conectar WebSocket con:', {
    ...
    async initializeWebphone() {
      try {
        console.log('Iniciando la configuración de JsSIP con:', {
          server: this.sipConfig.server,
        ...
        console.error('Error al inicializar el teléfono web:', error)
        this.$q.notify({
          color: 'negative',
          message: `Error al inicializar el teléfono web: ${error.message}`,
          icon: 'error'
        })
      }
    },
    ...
    handleIncomingCall(session) {
      ...
      session.on('peerconnection', (data) => {
        ...
        pc.oniceconnectionstatechange = () => {
          console.log('ICE Estado de la conexión (llamada entrante):', pc.iceConnectionState)
        }
        
        pc.onconnectionstatechange = () => {
          console.log('Estado de la conexión (llamada entrante):', pc.connectionState)
        }
    ...
    async handleCall() {
      ...
      try {
        ...
        this.session.on('peerconnection', (data) => {
          ...
          pc.oniceconnectionstatechange = () => {
            console.log('ICE Estado de la conexión (llamada saliente):', pc.iceConnectionState)
          }
          
          pc.onconnectionstatechange = () => {
            console.log('Estado de la conexión (llamada saliente):', pc.connectionState)
          }
  ...
  mounted() {
    ...
    const tryConnect = async (attempt = 1) => {
      const maxAttempts = 3
      try {
        console.log(`Intentar ${attempt} de ${maxAttempts} conexión WebSocket`)
        console.log('Usando configuración:', this.sipConfig)
        
        await this.testWebSocket()
        console.log('Prueba de WebSocket exitosa, iniciando JsSIP...')
        ...
        if (attempt < maxAttempts) {
          ...
          this.$q.notify({
            color: 'negative',
            message: 'La conexión de WebSocket falló después de varios intentos',
            icon: 'error',
            timeout: 5000
          })
        }
