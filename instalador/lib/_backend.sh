...
backend_db_create() {
  ...
  docker run -d --name portainer-$deploy_name \
   ...
   sleep 2
  docker start postgresql-$deploy_name
  docker update --restart=always postgresql-$deploy_name
  docker update --restart=always redis-$deploy_name
  docker update --restart=always portainer-$deploy_name
...
# Carpetas críticas del backend
  dirs=(
    "/home/$deploy_name/$deploy_name/backend/sessions"
    "/home/$deploy_name/$deploy_name/backend/.wwebjs_auth"
    "/home/$deploy_name/$deploy_name/backend/.wwebjs_cache"
    "/home/$deploy_name/$deploy_name/backend/uploads"
    "/home/$deploy_name/$deploy_name/backend/logs"
  )

  # Crear carpetas si no existen y dar permisos
  for dir in "\${dirs[@]}"; do
    mkdir -p "\$dir"
    chown -R $deploy_name:$deploy_name "\$dir"
    chmod -R 755 "\$dir"
  done
