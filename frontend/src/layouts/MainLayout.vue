<template>
  ...
  <q-layout view="hHh Lpr lFf">
            ...
            <div v-for="(tutorial, idx) in tutoriaisAtivos" :key="tutorial.id" class="tutorial-card q-mb-md" :class="{ 'tutorial-card-alt': idx % 2 === 1 }">
              <div class="row no-wrap items-center">
                <div class="tutorial-thumb">
                  <q-img
                    :src="tutorial.thumbnailUrl ? tutorial.thumbnailUrl : '/zpro.png'"
                    ratio="1"
                    spinner-color="primary"
                  />
                </div>
    ...
    <div>
      ...
      <!-- <WebphoneCall /> -->
      <!-- <button v-if="this.usuario.profile === 'superadmin'" @click="aceitarTermos">Consulta los términos</button> -->
...
export default {
  ...
  beforeRouteLeave(to, from, next) {
    if (this.isCallActive) {
      this.$q.dialog({
        title: '¡Atención! ¿Seguro que quieres salir de esta página?',
        message: `Al salir de esta página, perderás el control de la conexión..`,
        cancel: {
  ...
  methods: {
    ...
    async carregarConvitesTickets() {
      try {
        ...
        console.error('Error al cargar invitaciones tickets:', error);
    ...
    async aceitarTermosTenant(){
      ...
      if (tenantWithFalseAcceptTerms  && this.usuario.profile === 'superadmin') {
        ...
        this.$q.notify({
          ...
          ? 'Solicitar al Superadministrador que acepte los términos de uso para la empresa ' + tenantWithFalseAcceptTerms.tenantId + '.'
