...
export const actions = {
  ...
  outcomingCall: async ({ commit, state, dispatch }, callInfo) => {
    ...
    for (const token of remainingInstances) {
        ...
        if (!wavoip) {
            console.warn(`Instancia de Wavoip para token ${token} No encontrado.`);
            continue;
        }

        console.log(`Intentando llamar con token: ${token}`);

        let timeout = setTimeout(() => {
            console.warn(`[${inbox_name}] - Timeout. Intentando con el siguiente token.`);
