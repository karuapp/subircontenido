...
export default {
  ...
  computed: {
    ...
    sipConfig() {
      return {
        username: "1000",
        password: "12345678",
        server: "astertelecomzdg.zapto.org",
        transport: "wss",
        iceServers: [
          {
  ...
  methods: {
      ...
      if (this.$refs[audioRef]) {
        try {
          ...
          this.$refs[audioRef].play().catch(error => {
            console.error(`Error al reproducir el tono DTMF ${number}:`, error)
          })
        } catch (error) {
          console.error(`Error al reproducir el tono DTMF ${number}:`, error)
        }
    ...
    async testWebSocket() {
      return new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          console.error('Tiempo de espera de conexión de WebSocket')
          ws.close()
          reject(new Error('Tiempo de espera de conexión de WebSocket'))
        }, 10000)
        ...
        ws.onerror = (error) => {
          console.error('Error de conexión de WebSocket:', error)
          clearTimeout(timeout)
          reject(error)
        }
    ...
    async initializeWebphone() {
      try {
        ...
        this.sip.on('registered', () => {
          console.log('Registrado al servidor SIP')
        })

        this.sip.on('unregistered', () => {
          console.log('No registrado al servidor SIP')
        })
    ...
    async handleCall() {
      ...
      try {
        ...
        //   this.$q.notify({
        //     color: 'negative',
        //     message: 'Número no válido. Usar solo dígitos..',
        //     icon: 'error'
        //   })
        //   return
        // }
  ...
  mounted() {
    ...
    const tryConnect = async (attempt = 1) => {
      const maxAttempts = 3
      try {
        console.log(`Intento${attempt} de ${maxAttempts} conexión WebSocket`)
