<template>
  <div v-if="easyFlowVisible"
    ...
    <!-- VResultado de la visualización -->
...
export default {
  ...
  methods: {
    ...
    jsPlumpConsist (evt) {
      ...
      */
      this.$notificarSucesso('Conexión exitosa.')
      return true
    },
    jsPlumbInit () {
      this.jsPlumb.ready(() => {
    ...
    deleteElement () {
      ...
      if (this.activeElement.type === 'node') {
        ...
        this.$q.dialog({
          ...
          message: '¿Seguro que desea eliminar la línea seleccionada?',
          cancel: {
    ...
    deleteNode (node) {
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `¿Seguro que quieres eliminar el artículo? (${node.name})?`,
        cancel: {
          label: this.$t('common.no'),
          color: 'primary',
          push: true
    ...
    downloadData () {
      this.$q.dialog({
        ...
        this.$notificarSucesso('Descargando, espera...')
      })
    },
    openHelp () {
