...
export default {
  ...
  methods: {
    ...
    async testWebhook() {
      ...
      try {
        ...
        const testData = {
          ...
          kanban: 'Cliente',
          user: 'Asistente',
          userEmail: 'atendente@empresa.com',
          firstName: 'John',
          lastName: 'Smith',
          businessName: 'Ejemplo de empresa Ltd.'
        };
        
        // Preparar la URL con las variables reemplazadas
        ...
        // Preparar el cuerpo para POST/PUT
        let body = undefined;
        if (['POST', 'PUT'].includes(this.$attrs.element.data.httpMethod) && this.$attrs.element.data.bodyContent) {
          if (this.$attrs.element.data.bodyType === 'json') {
            try {
              // Reemplazar las variables en el cuerpo JSON
            ...
            // Reemplazar las variables en el cuerpo del formulario
            let bodyContent = this.$attrs.element.data.bodyContent;
            Object.keys(testData).forEach(key => {
              bodyContent = bodyContent.replace(new RegExp(`{{${key}}}`, 'g'), testData[key]);
            });
        ...
        // Configuración de la solicitud de prueba
        const fetchOptions = {
          ...
          // Añadir credenciales para una mejor compatibilidad
          credentials: 'omit'
        };
        
        // Añadir el cuerpo solo para POST/PUT
        ...
        // Ejecutar la solicitud de prueba usando la búsqueda nativa con gestión CORS
        let response;
        let responseData = {};
        
        try {
          // Primer intento: búsqueda normal
          ...
          // Si falla, pruebe con el modo "no-cors" para comprobar si se trata de un problema de CORS.
          try {
            const noCorsOptions = {
              ...fetchOptions,
              mode: 'no-cors'
            ...
            // Con "no-cors", no podemos leer la respuesta, pero sabemos que llegó.
            this.testResult = {
              success: true,
            ...
            // Si sigue fallando, es un error real.
            throw fetchError;
          }
        }
    ...
    initializeData() {
      // Asegurarse de que $attrs.element.data exista
      if (!this.$attrs.element.data) {
        this.$attrs.element.data = {};
      }
      
      // Inicializar los datos predeterminados si no existen
