  ...
  computed: {
    cMediaUrl () {
      if (this.$attrs.element.data?.mediaUrl) {
        let mediaUrl = this.$attrs.element.data.mediaUrl
        
        // Se é uma URL completa (contém http), ajustar o caminho para chatbot
        if (mediaUrl.includes('http') && mediaUrl.includes('/public/')) {
          // Extrair o nome do arquivo da URL
          const urlParts = mediaUrl.split('/')
          const fileName = urlParts[urlParts.length - 1]
          const tenantId = urlParts[urlParts.length - 2] // Pega o ID do tenant
          
          // Reconstruir a URL com /chatbot/ no caminho mantendo o tenant ID
          const publicIndex = urlParts.findIndex(part => part === 'public')
          mediaUrl = `${urlParts.slice(0, publicIndex + 1).join('/')}/${tenantId}/chatbot/${fileName}`
        }
