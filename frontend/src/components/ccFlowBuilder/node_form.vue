              ...
              <div v-show="type === 'node'">
                ...
                <div class="row q-mt-md q-mb-md justify-center">
                  <q-card flat class="q-pa-md bg-grey-2" style="min-width: 400px;">
                    ...
                    <div class="row q-gutter-sm">
                      <!-- Campo para la clave de la variable -->
                      <div class="col">
                        <q-input
                          dense
                          outlined
                      ...
                      <!-- Campo para el nombre descriptivo de la variable -->
                      ...
                      <!-- Campo para el tipo de variable -->
                      <div class="col" v-if="node.variableKey">
                        <q-select
                          dense
                          outlined
                      ...
                      <!-- Campo para la validación personalizada -->
            ...
            <div class="row">
              <div class="col">
                ...
                <q-input
                  ...
                  :value="node.configurations.welcomeMessage.message"
                >
                  <!-- Botón de emoji dentro de la entrada -->
        ...
        <!-- <q-card class="full-width q-my-sm"
          ...
          <q-card-section class="q-pa-sm">
            <div class="row q-mt-sm">
              <div class="col">
                <label class="text-subtitle1 text-bold q-mb-sm"
                  for="inputEnvioMensagem"> Palabra de activación: </label>
                <div class="flex flex-inline full-width items-center">
...
export default {
  ...
  methods: {
    ...
    addLineStep (nextStepId, idx) {
      console.log('addLineStep called:', { nextStepId, idx, nodeId: this.node.id })
      ...
      console.log('addLineStep - oldToLine:', oldToLine, 'nextStepId:', nextStepId)
      ...
      console.log('Emitting addNewLineCondition event:', { from: this.node.id, to: nextStepId, oldTo: oldToLine })
    ...
    nodeInit (data, id) {
      ...
      if (foundNode) {
        // Crear una copia profunda para evitar referencias directas
        this.node = cloneDeep(foundNode)
        // Asegurar la existencia de propiedades
        ...
        if (!this.node.configurations) this.node.configurations = {
          ...
          keyword: { message: '' },
          notResponseMessage: { message: '', time: 0, type: 1, destiny: null },
        ...
        // Asegurar las propiedades de las variables
    ...
    // Método para sincronizar los cambios con el objeto original
    syncNodeChanges() {
      if (!this.node || !this.node.id) return
      
      const originalNode = this.data.nodeList.find(n => n.id === this.node.id)
      if (originalNode) {
        // Sincronizar todas las propiedades importantes
        ...
        // Evento de grabación para repintar si es necesario
    ...
    // Método para guardar con sincronización
    saveFlowWithSync() {
      // Sincronizar todos los cambios antes de guardar
      this.syncNodeChanges()
      // Evento de grabación y guardado
      this.$emit('saveFlow')
    },
    
    // Método para sincronizar cuando se pierde el foco (antirrebote)
    syncOnBlur() {
      // Usar antirrebote para evitar múltiples llamadas
      if (this.syncTimeout) {
        clearTimeout(this.syncTimeout)
      }
    ...
    // Método para actualizar los datos de un elemento específico
    ...
    // Método para restaurar datos de la caché
    ...
    // Métodos para comparaciones de texto
    ...
    clearActionFields(condition, idx, actionType) {
      // Borrar todos los campos de acción excepto el seleccionado
      ...
      // Borrar el campo de condición solo si no es de tipo 'R' (respuestas)
      if (condition.type !== 'R') {
     ...
     // Método para validar la clave de la variable del paso
     ...
     // Método para generar automáticamente una clave para el paso
     ...
     // Método para forzar la actualización de la interfaz (eliminado para mejorar el rendimiento)
     forceUpdateInterface() {
       // Método mantenido por compatibilidad, pero sin implementación
       // para evitar Fallos al escribir
     }
  },
  watch: {
    // Autocompletar clave de variable al cambiar el nombre del paso
    'node.name': {
      handler(newName) {
    ...
    // Observador de propiedades de variable
    'node.variableKey': {
      handler(newValue) {
    ...
    // Observador de interacciones (campos añadidos)
    'node.interactions': {
      handler() {
        this.syncNodeChanges()
      },
    ...
    // Observador del objeto nodo (simplificado)
    'node': {
      handler(newNode) {
        console.log('Node updated:', newNode.id)
        // No sincronizar aquí para evitar bucles infinitos
        // La sincronización se realiza en observadores específicos
      },
      deep: true
    },
    
    // Observador de cambio de pestaña
    'tabNodeForm': {
      handler(newTab, oldTab) {
        console.log('Pestaña cambiada desde', oldTab, 'to', newTab)
        // Sincronizar datos al cambiar de pestaña
        this.syncNodeChanges()
      }
    }
  },
