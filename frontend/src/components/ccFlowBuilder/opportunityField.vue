  ...
  <div>
    <q-card flat class="q-pa-sm q-pb-md" style="min-height: 40px;">
      <q-card-section class="q-pa-none">
        ...
        <!-- Contacto (se completará automáticamente con el contacto del ticket) -->
        <div class="q-mb-sm">
          <q-input
            :value="contactInfo"
...
export default {
  ...
  computed: {
    ...
    contactInfo() {
      // Se completará automáticamente con el contacto del ticket actual
    ...
    '$attrs.element.data': {
      handler(newData) {
        if (newData && Object.keys(newData).length > 0) {
          // No restablecer si ya existen datos válidos
          if (!newData.dateType) {
            this.initializeData();
          }
        }
  ...
  methods: {
    async carregarDados() {
      try {
        ...
        // Preparar opciones de etapa con el proceso de venta
    ...
    onStageChange(stageId) {
      ...
      if (stage) {
        // Actualizar automáticamente el pipelineId
    ...
    initializeData() {
      ...
      // Inicializa los datos predeterminados si no existen
      ...
      // Registrar para depurar
      console.log('OpportunityField - Datos inicializados:', this.$attrs.element.data);
    },
    ...
    onDateTypeChange(value) {
      if (value === 'days_from_interaction') {
        ...
        // Borrar los datos personalizados si ya no se necesitan
        this.$attrs.element.data.closingForecast = '';
      }
    }
  },
