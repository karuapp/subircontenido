  ...
  methods: {
    ...
    async getMediaUrl () {
      if (this.file?.type) {
        ...
        this.$set(this.$attrs.element.data, 'mediaUrl', this.file.name)
        this.$set(this.$attrs.element.data, 'media', await this.getBase64(this.file))
        this.$set(this.$attrs.element.data, 'ext', this.getFileExtension(this.file.name))
        this.$set(this.$attrs.element.data, 'type', this.file.type)
        this.$set(this.$attrs.element.data, 'name', this.file.name)
        
        // Forçar re-renderização
        this.$forceUpdate()
        
        // Limpar o arquivo para permitir nova seleção
        this.file = null
      }
    },
    
    deleteMedia() {
      // Limpar todos os dados de mídia usando $set para garantir reatividade
      this.$set(this.$attrs.element.data, 'mediaUrl', '')
      this.$set(this.$attrs.element.data, 'media', '')
      this.$set(this.$attrs.element.data, 'type', '')
      this.$set(this.$attrs.element.data, 'name', '')
      this.$set(this.$attrs.element.data, 'ext', '')
