  ...
  <div>
    <q-card flat class="q-pa-sm q-pb-md" style="min-height: 40px;">
      <q-card-section class="q-pa-none">
        ...
        <!-- Eliminando el antiguo botón de limpieza a continuación -->
      </q-card-section>
    </q-card>
  </div>
</template>
  ...
  methods: {
    async carregarDados() {
      try {
        ...
        console.error('Error al cargar datos VAPI:', error)
        this.$q.notify({
          type: 'negative',
          message: 'Error al cargar datos VAPI'
        })
      } finally {
        this.loading = false
      }
    ...
    initializeData() {
      if (!this.$attrs.element.data.assistantId) {
        this.$set(this.$attrs.element.data, 'assistantId', '')
      }
      if (!this.$attrs.element.data.phoneNumberId) {
        this.$set(this.$attrs.element.data, 'phoneNumberId', '')
      }
    },
    updateData() {
