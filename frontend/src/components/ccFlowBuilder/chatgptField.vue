          ...
          <div class="row q-mt-none q-mb-none">
            <div class="col">
              ...
              <q-select v-if="$attrs.element.data.routeAction === 0"
                ...
                outlined
                class="full-width"
                :value="$attrs.element.data.nextStepId || ''"
                ...
                emit-value
                clearable
                @input="nextStepId => updateRouteAction('step', nextStepId)" />
              ...
              <q-select v-if="$attrs.element.data.routeAction === 4"
                ...
                map-options
                clearable
                @input="() => updateRouteAction('channel')" />
            </div>
          </div>
        </q-card>
...
export default {
  ...
  methods: {
    updateRouteAction(actionType, value = null) {
      ...
      if (!this.$attrs.element.data.routeAction) {
        this.$set(this.$attrs.element.data, 'routeAction', 0)
      }
      
      // Inicializa el tipo de comparación si no existe
      if (!this.$attrs.element.data.routeComparisonType) {
        this.$set(this.$attrs.element.data, 'routeComparisonType', 'equals')
      }
      
      // Borra los demás campos de acción excepto el seleccionado
      if (actionType !== 'step') this.$set(this.$attrs.element.data, 'nextStepId', null)
      if (actionType !== 'queue') this.$set(this.$attrs.element.data, 'queueId', null)
      if (actionType !== 'user') this.$set(this.$attrs.element.data, 'userIdDestination', null)
      if (actionType !== 'close') this.$set(this.$attrs.element.data, 'closeTicket', null)
      if (actionType !== 'channel') this.$set(this.$attrs.element.data, 'channelDestination', null)
      
      // DEstablece el valor específico si se proporciona
      if (value !== null) {
        if (actionType === 'step') {
          this.$set(this.$attrs.element.data, 'nextStepId', value)
        }
      }
      
      // Establece el chatFlowId según routeAction
      this.updateChatFlowId()
    },
    
    updateChatFlowId() {
      // Si se configura una acción de enrutamiento, configúrela chatFlowId
      if (this.$attrs.element.data.routeAction !== undefined && this.$attrs.element.data.routeAction !== null) {
        ...
        if (!this.$attrs.element.data.chatFlowId) {
          // Si no se define un chatFlowId, se deja en nulo para que lo configure el backend
          this.$set(this.$attrs.element.data, 'chatFlowId', null)
        }
      } else {
  ...
  mounted() {
    ...
    // Actualiza el chatFlowId según configuración actual
    this.updateChatFlowId()
  }
}
</script>
