  ...
  <div class="asterisk-webphone">
    <div v-if="isConnected" class="webphone-container">
      ...
      <div class="call-controls">
        ...
        <template v-if="isCalling && callStatus === 'Chamada recebida'">
          <button 
            class="call-button"
...
export default {
  ...
  methods: {
    ...
    async handleIncomingCall(invitation) {
      ...
      const usuario = JSON.parse(localStorage.getItem('usuario'));
      if (!this.phoneNumber) {
        console.error('Número de teléfono no definido al intentar crear un registro de llamadas.');
        return;
      }
      await CriarCallLog({
        userId: usuario.userid,
    ...
    async hangupCall() {
      ...
      try {
        ...
        this.session = null;
        console.log('Estado de la llamada borrado');
      }
    },
    playDialTone(number) {
      let audioRef;
