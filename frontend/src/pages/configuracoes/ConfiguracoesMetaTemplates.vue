  ...
  methods: {
    ...
    async editarTemplateSubmit() {
      try {
        ...
        if (error.response?.data?.error === 'La plantilla de muestra no se puede editar') {
          this.$q.notify({
            type: 'warning',
    ...
    async confirmarExclusao(template) {
      this.$q.dialog({
        ...
        try {
          ...
          // Verificar se é um erro específico sobre templates de exemplo
          if (error.response?.data?.error === 'La plantilla de muestra no se puede eliminar') {
            this.$q.notify({
              type: 'warning',
    ...
    async abrirModalWebhook() {
      ...
      try {
        ...
        if (!metaToken) {
          this.$q.notify({
            type: 'negative',
            message: 'No se encontró el metatoken. Configure el token en la sección de configuración..',
            timeout: 5000
          })
          return
        }
        ...
        console.error('Error al obtener metaToken:', error)
        this.$q.notify({
          type: 'negative',
          message: 'Error al obtener la configuración del inquilino. Inténtelo de nuevo..',
          timeout: 5000
        })
      }
    },
    ...
    async sobrescreverWebhook() {
      try {
        ...
        this.$q.notify({
          type: 'negative',
          message: error.response?.data?.error || 'Error al configurar el webhook. Compruebe los datos y vuelva a intentarlo..',
          timeout: 5000
        })
      } finally {
        this.loadingWebhook = false
