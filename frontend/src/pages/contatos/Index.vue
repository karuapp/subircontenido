      ...
      <template v-slot:body-cell-acoes="props">
        <q-td class="text-center">
          ...
          <q-btn flat round icon="img:meow-logo.png" @click="handleSaveTicket(props.row, 'meow')"
            ...
            <q-btn flat round icon="img:zapi-logo.png" @click="handleSaveTicket(props.row, 'zapi')"
            v-if="props.row.number && cSessionsZapi.length > 0" />
            <q-btn flat round icon="img:uazapi-logo.png" @click="handleSaveTicket(props.row, 'uazapi')"
            v-if="props.row.number && cSessionsUazapi.length > 0" />
              ...
              <q-select square borderless v-model="tags" multiple :options="etiquetas" use-chips option-value="id"
                ...
                <template v-slot:option="{ itemProps, itemEvents, opt, selected, toggleOption }">
                  <q-item v-bind="itemProps" v-on="itemEvents">
                    ...
                    <q-item-section side>
                      <q-checkbox :value="selected" @update:model-value="toggleOption(opt)" />
                    </q-item-section>
                  </q-item>
                </template>
  ...
  methods: {
    ...
    async bloquearChatbot(contact, bloquearStatusChatbot) {
      ...
      try {
        ...
        console.error('Error al bloquear el chatbot:', error);
        this.$q.notify({
          type: 'negative',
    ...
    formatTags(tags) {
      ...
      downloadModelCsv() {
      const csvContent =
        'nombre;número;email;identificación;fecha_nacimiento;primer_nombre;apellido;empresa\n' +
        'Usuario Ejemplo;50498765432;usuario@mail.com;00000000;15/06/1987;Andrés;López;MiEmpresa';

      const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
      const link = document.createElement('a');
      link.href = window.URL.createObjectURL(blob);
      link.setAttribute('download', 'modelo_contactos.csv');
      ...
      this.$q.dialog({
        ...
        for (const contactId of this.selectedContacts) {
          try {
            ...
            console.error(`Error al eliminar el contacto ${contactId}:`, error);
            ...
            if (errorMessage === 'ERR_CONTACT_TICKETS_REGISTERED' || errorMessage === 'ERR_CONTACT_OPPORTUNITIES_REGISTERED') {
              ...
              if (forceDelete) {
                try {
                  ...
                  console.error(`Error al eliminar el contacto forzosamente ${contactId}:`, forceError);
    ...
    showBulkDeleteResults(results) {
      ...
      let message = `Procesado ${totalProcessed} contactos:\n`;
      if (successCount > 0) message += `✅ ${successCount} Eliminado correctamente\n`;
      if (forceDeletedCount > 0) message += `⚠️ ${forceDeletedCount} Eliminado forzado\n`;
      if (failedCount > 0) message += `❌ ${failedCount} Error`;
    ...
    abrirAtendimentoExistente(contato, ticket) {
      ...
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `${this.$t('contacts.notifications.existingTicketDetails')}: Ticket #${ticket.id} - User #${ticket.userId} - ${ticket.user.name}`,
        cancel: {
          label: this.$t('common.no'),
          color: 'primary',
          push: true
