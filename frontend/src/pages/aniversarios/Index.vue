...
export default {
  ...
  computed: {
    ...
    cSessions() {
      return this.whatsapps.filter(w => (w.type === 'whatsapp' || w.type === 'waba' || w.type === 'baileys' || w.type === 'evo' || w.type === 'meow' || w.type === 'uazapi' || w.type === 'zapi')  && !w.isDeleted && w.status === 'CONNECTED' && w.birthdayDate === 'enabled');
    },
    cSessionsOptions() {
    ...
    async sendBirthdayMessage() {
      ...
      if (this.selectedWhatsApp) {
        try {
          ...
          this.$q.notify({
            type: 'negative',
            message: 'Error al enviar los mensajes de cumpleaños.'
          });
        }
      }
