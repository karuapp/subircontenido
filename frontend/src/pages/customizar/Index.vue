          ...
          <q-card flat bordered class="my-card col-12 col-md-3">
            ...
            <q-card-section class="q-pa-sm">
              <div class="text-center" style="height: 60px;">
                ...
                <div v-else class="text-grey">No se ha seleccionado ningún vídeo</div>
              </div>
            </q-card-section>
            <q-card-section class="q-pa-sm">
              <q-input filled dense style="width: 100%;">
  ...
  methods: {
    ...
    async loadTenantsList() {
      try {
        ...
        if (response.status === 200) {
          ...
          console.error('Error al cargar la lista de empresas');
        }
      } catch (error) {
    ...
    async loadTenantForceLogoutConfig() {
      ...
      try {
        ...
        console.error('Error al cargar la configuración de cierre de sesión forzado:', error);
      }
    },
    async updateForceLogoutConfig() {
      if (!this.selectedTenant) {
        console.log('No se seleccionó ningún inquilino');
        return;
      }
      ...
      try {     
        ...
        if (response.status === 200) {
          this.$q.notify({
            type: 'positive',
            message: 'La configuración de cierre de sesión forzado se actualizó correctamente',
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
        ...
        console.error('=== ERROR ACTUALIZAR CIERRE DE SESIÓN FORZADO ===');
        console.error('Detalles del error:', error);
        console.error('Respuesta al error:', error.response);
        this.$q.notify({
          type: 'negative',
          message: 'Error al actualizar la configuración de cierre de sesión forzado',
          position: 'top',
    ...
    async loadMasterkeyConfig() {
      try {
        ...
        if (response.status === 200) {
          ...
          console.error('Error al cargar la configuración de la llave maestra');
        }
      } catch (error) {
        console.error('Error al cargar la configuración de la llave maestra:', error);
      }
    },
    async updateMasterkeyConfig() {
      try {
        ...
        if (response.status === 200) {
          this.$q.notify({
            type: 'positive',
            message: 'La configuración de la llave maestra se actualizó correctamente',
            position: 'top',
            timeout: 3000,
            actions: [{
              icon: 'close',
        ...
        console.error('Error al actualizar la configuración de la llave maestra:', error);
        this.$q.notify({
          type: 'negative',
          message: 'Error al actualizar la configuración de la llave maestra',
          position: 'top',
    ...
    async loadEncryptionkeyConfig() {
      try {
        ...
        if (response.status === 200) {
          ...
          console.error('Error al cargar la configuración de la clave de cifrado');
        }
      } catch (error) {
        console.error('Error al cargar la configuración de la clave de cifrado:', error);
      }
    },
    async updateEncryptionkeyConfig() {
      try {
        ...
        console.error('Error al actualizar la configuración de la clave de cifrado:', error);
        this.$q.notify({
          type: 'negative',
          message: this.$t('customizar.encryptionkey.errorUpdate'),
          position: 'top',
