<template>
  <div class="q-my-md" v-if="(userProfile === 'admin')">
    <q-card flat bordered class="my-sticky-dynamic q-ma-lg">
      <q-card-section>
        <div class="text-h5">{{ $t('callLogs.title') || 'Registros de llamadas' }}</div>
        <div class="text-caption">{{ $t('callLogs.subtitle') || 'Registros de llamadas entrantes y salientes' }}</div>
      </q-card-section>
      <q-card-section>
  ...
  methods: {
    async fetchCallLogs() {
      try {
        ...
        console.error('Error al recuperar los registros de llamadas:', error);
      }
    },
    formatDate(date) {
