<template>
  <div class="q-my-md" v-if="userProfile === 'admin'">
    ...
    <q-card flat bordered class="my-sticky-dynamic q-ma-lg" v-if="wavoipAuth">
      <q-card-section>
        <div class="text-h5">{{ $t('wavoip.logsTitle') || 'Registros de eventos de WebSocket' }}</div>
        <div class="text-caption">{{ $t('wavoip.logsSubtitle') || 'Eventos y llamadas registrados por el sistema' }}</div>
      </q-card-section>
      <q-card-section class="q-pa-none">
        <q-expansion-item
          icon="mdi-information-outline"
          :label="$t('wavoip.eventSummary') || 'Resumen de eventos'"
        >
            <q-card>
              <q-card-actions align="right">
              <q-btn flat dense round @click="downloadSocketEvents" icon="mdi-download" class="q-mr-sm">
                <q-tooltip>{{ $t('wavoip.downloadLogs') || 'Descargar registros de eventos' }}</q-tooltip>
              </q-btn>
              <q-btn flat dense round @click="clearSocketEventsWithConfirmation" icon="mdi-delete-sweep" class="q-mr-sm">
                <q-tooltip>{{ $t('wavoip.clearLogs') || 'Borrar registros de eventos' }}</q-tooltip>
              </q-btn>
            </q-card-actions>
            </q-card>
            <q-card>
                ...
                <div class="col-12 col-md-4">
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{ $t('wavoip.connectEvents') || 'Conexiones' }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </div>
                <div class="col-12 col-md-4">
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{ $t('wavoip.incomingEvents') || 'Llamadas entrantes' }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </div>
                <div class="col-12 col-md-4">
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{ $t('wavoip.outgoingEvents') || 'Llamadas salientes' }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </div>
                <div class="col-12 col-md-4">
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{ $t('wavoip.errorEvents') || 'Errores' }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </div>
                <div class="col-12 col-md-4">
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{ $t('wavoip.offer') || 'Ofertas' }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </div>
                <div class="col-12 col-md-4">
                  <q-item>
                    ...
                    <q-item-section>
                      ...
                      <q-item-label caption>{{ $t('wavoip.terminate') || 'Terminaciones' }}</q-item-label>
                    </q-item-section>
                  </q-item>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    downloadSocketEvents() {
      ...
      this.$q.notify({
        message: this.$t('wavoip.logsDownloaded') || 'Registros de eventos descargados correctamente',
        color: 'positive',
        icon: 'mdi-download'
      });
    },
    ...
    clearSocketEventsWithConfirmation() {
      this.$q.dialog({
        title: this.$t('wavoip.confirmClear') || 'Borrar registros',
        message: this.$t('wavoip.confirmClearMessage') || '¿Seguro que desea borrar todos los registros de eventos?',
        cancel: true,
        persistent: true
      }).onOk(() => {
        ...
        this.$q.notify({
          message: this.$t('wavoip.logsCleared') || 'Registros de eventos borrados correctamente',
          color: 'positive',
