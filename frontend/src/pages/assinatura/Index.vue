  ...
  <div v-if="userProfile === 'superadmin'">
    <q-page padding>
        ...
        <q-card-section class="q-pa-md">
          <q-list separator>
            ...
            <q-item>
              ...
              <!-- <q-item-section side>
                <q-icon name="mdi-security" color="primary" size="md" >
                  <q-tooltip>
                    {{ product_name && typeof product_name === 'string' && product_name.includes('ZPRO_') ? product_name.split('ZPRO_')[1] : '' }}
                  </q-tooltip>
                </q-icon>
              </q-item-section> -->
            </q-item>
      ...
      <q-dialog v-model="isDomainModalOpen" persistent>
        <q-card>
          <q-card-section>
            <div class="text-h6 text-primary">{{ $t('assinatura.domainModalTitle') || 'Editar dominios' }}</div>
            <div class="text-subtitle2 q-mt-sm">{{ $t('assinatura.domainModalSubtitle') || 'Actualizar dominios con licencia' }}</div>
          </q-card-section>

          <q-card-section>
            <q-input
              v-model="domainInput"
              :label="$t('assinatura.domainFieldLabel') || 'Dominios (separados por comas)'"
              outlined
              dense
              type="textarea"
              :rules="[val => !!val || $t('assinatura.domainFieldError') || 'Dominios obligatorios']"
...
export default {
  ...
  methods: {
    ...
    formatarDataBR(dataISO) {
      if (!dataISO) return "No se ha establecido la fecha";
