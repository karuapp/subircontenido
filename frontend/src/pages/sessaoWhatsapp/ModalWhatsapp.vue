        ...
        <q-card-section>
          <div class="row">
            ...
            <div class="col-12"
              ...
              <div class="q-tabs row items-center">
                <div class="q-tab" :class="{ 'q-tab--active': activeTab === 'tab1' }" @click="activeTab = 'tab1'" :style="{ backgroundColor: $q.dark.isActive ? '#363636' : '#f2f2f2', borderRadius: '10px 10px 0 0', textTransform: 'capitalize' }">Información</div>
                ...
                <fieldset class="full-width q-pa-md">
                  ...
                  <div class="col-12"
                    ...
                    <c-input filled
                      ...
                      hint="Contraseña usada para iniciar sesión en Instagram"
                      placeholder="*************"
                      :disable="!isEdit">
              ...
              <fieldset class="full-width q-pa-md">
                ...
                <div class="q-mb-md">
                  <q-banner :class="connectionStatusClass" class="q-mb-md">
                    ...
                    <div class="q-mt-sm text-caption">
                      <strong>{{ $t('sessaoModalWhatsapp.novo.protocoloAtivo') }}</strong> {{ connectionStatus.workingProtocol?.toUpperCase() || 'NINGUNO' }}
                    </div>
          ...
          <div v-if="whatsapp.type === 'webmail'" class="q-mt-md">
            <fieldset class="q-pa-md" style="border: 1px solid #ccc; border-radius: 8px;">
              <legend class="q-px-sm" style="font-weight: bold; color: #1976d2;">Configuración de IMAP (Recepción de correos electrónicos)</legend>
              ...
              <c-input
                ...
                :rules="[val => !!val || 'Servidor IMAP requerido']"
              />
              ...
              <c-input
                ...
                type="number"
                placeholder="Ej.: 993"
                :rules="[val => !!val || 'Puerto IMAP requerido']"
              />
              
              <q-toggle
                v-model="whatsapp.smtpConfig.imap.tls"
                label="Conexión IMAP TLS"
                color="primary"
              />
            </fieldset>
          </div>
...
export default {
  ...
  computed: {
    ...
    connectionStatusText() {
      if (this.connectionStatus.workingProtocol === 'none') return 'No funciona la conexión';
      if (this.connectionStatus.workingProtocol === 'imap') return 'IMAP funciona';
      if (this.connectionStatus.workingProtocol === 'pop3') return 'POP3 funciona (alternativa)';
  ...
  methods: {
    ...
    async handleSaveWhatsApp (whatsapp) {
      ...
      if (whatsapp.type === 'webmail' && this.whatsAppEdit.id) {
        try {
          ...
          console.error('Error al recargar los datos de autenticación OAuth2 desde el correo web:', error);
        }
      }
      ...
      if(whatsapp.type !== 'hub'){
        ...
        if (!this.selectedHubOption) {
          ...
          return this.$q.notify({
            type: 'warning',
            message: 'Selecciona un Hub antes de continuar.',
            position: 'top',
            actions: [{ icon: 'close', round: true, color: 'white' }]
          });
        }
        ...
        try {
          ...
          this.$q.notify({
            type: 'positive',
            message: 'Hub ¡Añadido correctamente!',
            position: 'top'
          });
          this.$emit('recarregar-lista')
          this.fecharModal();
          ...
          console.error('Error al añadir Hub:', error);
          this.loading = false // Reset loading em caso de erro
          this.$q.notify({
            type: 'negative',
            message: 'Error al añadir el Hub. Inténtalo de nuevo.',
            position: 'top'
          });
        }
      }
    ...
    async configureGmailOAuth2Method() {
      try {
        ...
        if (!this.whatsapp.id) {
          throw new Error('No se encontró el ID de WhatsApp. Guarde WhatsApp primero..');
        }
        ...
        if (!this.whatsapp.smtpConfig.oauth2?.client_id || !this.whatsapp.smtpConfig.oauth2?.client_secret) {
          throw new Error('Configure primero el ID de cliente y el secreto de cliente de OAuth2..');
        }
        ...
        console.error('Error al configurar OAuth2.:', error);
        this.$q.notify({
          type: 'negative',
    ...
    async completeOAuth2SetupMethod(code) {
      try {
        ...
        // Verificar se tem WhatsApp ID
        if (!this.whatsapp.id) {
          throw new Error('No se encontró el ID de WhatsApp. Guardar WhatsApp primero..');
        }
        ...
        console.error('Error al completar OAuth2.:', error);
        this.$q.notify({
          type: 'negative',
    ...
    async testImapConnectionMethod() {
      try {
        ...
        if (!this.whatsapp.id) {
          this.$q.notify({
            type: 'warning',
            message: 'Guardar WhatsApp primero para probar la conexión IMAP.',
            position: 'top'
          });
          return;
        }
        ...
        if (response.data.success) {
          ...
          this.$q.notify({
            type: 'negative',
            message: 'Prueba IMAP fallida.: ' + (response.data.message || 'Error desconocido.'),
            position: 'top'
          });
        }
      } catch (error) {
        console.error('Error al probar IMAP.:', error);
        this.$q.notify({
          type: 'negative',
    ...
    async testPop3ConnectionMethod() {
      try {
        ...
        if (!this.whatsapp.id) {
          this.$q.notify({
            type: 'warning',
            message: 'Guardar WhatsApp primero para probar la conexión POP3.',
            position: 'top'
          });
          return;
        }
        ...
        if (response.data.success) {
          ...
          this.$q.notify({
            type: 'negative',
            message: 'Prueba POP3 fallida.: ' + (response.data.message || 'Error desconocido.'),
            position: 'top'
          });
        }
      } catch (error) {
        console.error('Error al probar POP3.:', error);
        this.$q.notify({
          type: 'negative',
          message: 'Error al probar POP3.: ' + (error.response?.data?.message || error.message),
          position: 'top'
        });
      } finally {
  ...
  async mounted() {
    ...
    if (storedColors) {
      ...
      console.warn('No hay color almacenado. localStorage');
    }
    // this.loadColors()
