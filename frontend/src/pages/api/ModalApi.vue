...
export default {
  ...
  computed: {
    ...
    cSessions () {
      return this.whatsapps.filter(w => (w.type === 'whatsapp' || w.type === 'waba' || w.type === 'baileys' || w.type === 'evo' || w.type === 'meow' || w.type === 'uazapi' || w.type === 'zapi') && !w.isDeleted)
    },
    isWabaSelected () {
  ...
  methods: {
    ...
    async handleAPI () {
      this.v$.api.$touch()
      if (this.v$.api.$error) {
        this.$notificarErro('Revisar campos obligatorios e inconsistencias.')
        return
      }
      try {
        this.loading = true
