    ...
    >
      <q-card-section>
        ...
        <div class="row text-caption">
          {{ $t("contatosCampanha.start") }}: {{ formatDate($route.params.campanha.start) }} - {{ $t("contatosCampanha.status") }}: {{ campanha?.status }}
        </div>
        <q-btn
...
export default {
  ...
  computed: {
    campanha () {
      return this.$route?.params?.campanha || null
    },
    campanhaId () {
  ...
  mounted () {
    ...
    // (Opcional) tente preencher header se só tiver id
    // Se você tiver um endpoint para buscar a campanha por id, pode fazer aqui.
    // Exemplo comentado:
    // if (!this.campanha && this.campanhaId) {
    //   axios.get(`/api/campaigns/${encodeURIComponent(this.campanhaId)}`)
    //     .then(({ data }) => { this._campanhaHeader = data })
    //     .catch(() => {})
    // }
