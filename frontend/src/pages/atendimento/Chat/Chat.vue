  ...
  <div class="bg-white no-scroll hide-scrollbar overflow-hidden" :style="style">
    ...
    <q-footer class="bg-white">
      ...
      <q-banner class="text-grey-8" v-if="mensagensParaEncaminhar.length > 0">
        ...
        <!-- <span class="text-bold text-h5"> {{ mensagensParaEncaminhar.length }} De 10 mensajes</span>
        seleccionados para reenviar. -->
        <q-separator class="bg-grey-4" />
        <q-select
          dense
          class="q-my-md"
...
export default {
  ...
  watch: {
    // Borra la selección solo al cambiar de ticket.
    ticketFocado: {
    ...
    // Mantiene una copia siempre sincronizada.
    replyingMessage: {
      handler(newVal) {
    ...
    // Al actualizar la lista de mensajes, restaura la selección si es necesario.
    mensagensTicket: {
      handler() {
        if (this.preservedReplyingMessage && !this.replyingMessage) {
          this.$nextTick(() => {
  ...
  methods: {
    ...
    // Añade este método dentro de métodos.: { ... } en Chat.vue
    handlerNotifications(data) {
      // Si la notificación no corresponde al ticket que tiene el foco, no se realiza ninguna acción..
