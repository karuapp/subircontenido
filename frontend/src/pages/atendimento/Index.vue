                  ...
                  <div>
                    ...
                    >
                      <template v-slot:title>
                        <div :class="{ 'text-white': $q.dark.isActive }" style="width: calc(100% - 20px)">
                          <div class="row col text-bold text-body2">{{ (log.user && log.user.name) || 'Bot' }}:</div>
                          <div class="row col" :style="{ 'white-space': 'pre-line' }">{{ (log.notes) || 'Nota no registrada correctamente' }}</div>
                        </div>
                      </template>
                      <q-btn
                        flat
...
export default {
    ...
    notificacaoChatPrivado: {
      handler() {
        ...
        this.$nextTick(() => {
          ...
          if (isRecording) {
            console.log('El sonido no se reproduce durante la grabación')
            return
          }
    ...
    pendingTickets() {
      ...
      remainingTickets.forEach(ticket => {
        if (ticket.status === 'pending') {
          AtualizarStatusTicketNull(ticket.id, 'closed', ticket.userId);
          console.log(`Ticket duplicado #${ticket.id} encontrado.`);
        }
      });
  ...
  methods: {
    ...
    async atualizarLidContato() {
      ...
      try {
        ...
        this.$q.notify({
          ...
          caption: `Message: ${result.data.result.consolidationResults.totalMessagesMigrated}, Ticket: ${result.data.result.consolidationResults.totalTicketsMigrated}, Removed: ${result.data.result.consolidationResults.totalRemoved}`
        });
      } catch (error) {
        console.error('Erro ao atualizar LID:', error);
        this.$q.notify({
          type: 'negative',
      ...
      if (ticketExistente && ticketExistente.id) {
        ...
        if (ticket && ticket.id) {
          ...
          this.$q.notify({
            color: 'negative',
            position: 'top',
            message: 'Error: Ticket creado, pero sin ID válido.',
          });
        }
      }
    },
    ...
    async downloadPDF() {
      ...
      try {
        ...
        mensagens.forEach((mensagem) => {
          ...
          const remetente = mensagem.fromMe ? "Eu" : mensagem.contact.name || "Contacto";
    ...
    async sendEvaluation (ticketFocado) {
      this.$q.dialog({
        title: '¿Le gustaría enviar una reseña de este servicio? ',
        // message: 'Esse protocolo será armazenado no banco dados .',
        ok: {
          label: this.$t('common.yes'),
          color: 'primary',
        ...
        if (data) {
          this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: 'Reseña enviada a atención al cliente ' + ticketFocado.id,
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
          ...
          this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: 'No se puede enviar la Reseña del servicio.',
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
    ...
    async consultarTickets (paramsInit = {}) {
      ...
      try {
        ...
        this.$notificarErro('Algo incorrecto', err)
        console.error(err)
      }
    ...
    removerListenersNotificacao() {
      console.log('Eliminando oyentes de notificación...');
