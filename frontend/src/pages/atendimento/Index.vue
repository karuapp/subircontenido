    ...
    <q-layout class="WAL__layout shadow-3" container view="lHr LpR lFr">
      ...
      <q-toolbar class="q-pr-none q-gutter-xs full-width" style="height: 64px">
        ...
        <q-toolbar v-show="toolbarSearch" class="row q-gutter-sm q-py-sm items-center">
          ...
          <q-btn-dropdown
            ...
            <q-list>
              ...
              <q-item clickable v-close-popup @click="showModalMensagem = true">
                <q-item-section avatar>
        ...
        <q-toolbar v-show="toolbarSearch" class="row q-gutter-sm q-py-sm items-center">
          ...
          <!-- <q-input :placeholder="$t('generalSupport.lookingFor')" v-model="pesquisaTickets.searchParam" dense outlined rounded type="search" class="col-grow" :debounce="700" @input="BuscarTicketFiltro()" style="flex: 1.95;">
            <template v-slot:append>
          ...
          <div class="q-pa-sm">
                ...
                <q-card-section class="q-pa-sm">
                  <q-item>
                    <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
                      <q-btn flat class="btn-outline btn-small" :label="$t('common.funil')" icon="mdi-send-check" @click="abrirOpportunityForm" />
                    </q-card-section>
                    <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
                      <q-btn flat class="btn-outline btn-small" :label="$t('generalSupport.opportunities')" icon="mdi-timeline-text-outline" @click="abrirModalOportunidades" />
                    </q-card-section>
                  </q-item>
                </q-card-section>
            </q-card>
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat v-if="ticketFocado.channel !== 'instagram' && ticketFocado.channel !== 'telegram' && desabilitarInputWebchat">
              <q-card-section class="q-pa-none">
                ...
                <q-card-section class="q-pa-sm">
                  <q-item>
                    <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
                      <q-btn flat class="btn-outline btn-small" :label="$t('common.send')" icon="mdi-send-check" @click="sendProtocol(ticketFocado)" :disable="desabilitarInput" />
                    </q-card-section>
                    <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat v-if="ticketFocado.channel !== 'instagram' && ticketFocado.channel !== 'telegram' && desabilitarInputWebchat">
              ...
              <q-card-section class="q-pa-sm">
                <q-item>
                  <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
                    <q-btn flat class="btn-outline btn-small" :label="$t('common.send')" icon="mdi-send-check" @click="sendEvaluation(ticketFocado)" :disable="desabilitarInput" />
                  </q-card-section>
                  <q-card-section class="text-bold q-pa-sm" style="flex: 1;">
            ...
            <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat v-if="desabilitarInputWebchat">
              ...
              <q-card-section class="q-pa-sm">
                <q-item>
                  <q-card-section class="text-bold q-pa-sm">
                    <q-btn 
                      v-if="ticketFocado.channel === 'baileys' && !ticketFocado.isGroup"
                      flat 
                      class="btn-outline btn-small" 
            ...
            <!-- <q-card class="bg-white btn-rounded q-mt-sm" style="width: 100%" bordered flat>
              <q-card-section class="q-pa-none">
                <q-item tag="label" v-ripple>
                  <q-item-section>
                    <q-item-label>Forzar Grupo</q-item-label>
                    <q-item-label caption>Cambio isGroup</q-item-label>
                  </q-item-section>
                </q-item>
              </q-card-section>
              <q-card-section class="q-pa-sm">
                  ...
                  <div>
                    ...
                    >
                      <template v-slot:title>
                        <div :class="{ 'text-white': $q.dark.isActive }" style="width: calc(100% - 20px)">
                          <div class="row col text-bold text-body2">{{ (log.user && log.user.name) || 'Bot' }}:</div>
                          <div class="row col">{{ (log.protocol) || 'Protocolo no enviado correctamente' }}</div>
                        </div>
                      </template>
                    </q-timeline-entry>
                  </div>
...
export default {
    ...
    notificacaoChatPrivado: {
      handler() {
        ...
        this.$nextTick(() => {
          ...
          if (isRecording) {
            console.log('No se reproduce el sonido durante la grabación')
            return
          }
    ...
    pendingTickets() {
      ...
      remainingTickets.forEach(ticket => {
        if (ticket.status === 'pending') {
          AtualizarStatusTicketNull(ticket.id, 'closed', ticket.userId);
          console.log(`Ticket doble #${ticket.id} encontrado.`);
        }
      });
  ...
  methods: {
    ...
    async atualizarLidContato() {
      ...
      try {
        ...
        this.$q.notify({
          ...
          caption: `Mensaje: ${result.data.result.consolidationResults.totalMessagesMigrated}, Ticket: ${result.data.result.consolidationResults.totalTicketsMigrated}, Remoto: ${result.data.result.consolidationResults.totalRemoved}`
        });
      } catch (error) {
        console.error('Error al actualizar LID:', error);
        this.$q.notify({
          type: 'negative',
    ...
    async emailTenant(){
      ...
      if (tenantWithoutLicense  && this.usuario.profile === 'superadmin') {
        console.log(`Empresa con licencia inválida: ${tenantWithoutLicense.tenantId}`);
        // this.$q.notify({
        //   type: 'negative',
    ...
    async sendProtocol (ticketFocado) {
      this.$q.dialog({
        ...
        const message = {
          ...
          fromMe: true,
          mediaUrl: '',
          body: `Protocolo de chat: ${protocol}`,
          scheduleDate: null,
          quotedMsg: null,
          sendType: 'protocol',
          idFront: uid(),
    ...
    async sendEvaluation (ticketFocado) {
      this.$q.dialog({
        title: '¿Desea enviar una reseña sobre este servicio? ',
        // message: 'Esse protocolo será armazenado no banco dados .',
        ok: {
          label: this.$t('common.yes'),
          color: 'primary',
        ...
        const ticketId = ticketFocado.id
        const evaluation = 'Reseña enviada';
        const message = {
          read: 1,
          fromMe: true,
          mediaUrl: '',
        ...
        if (data) {
          this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: 'Reseña enviada sobre el chat ' + ticketFocado.id,
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
          ...
          this.$q.notify({
            ...
            progress: true,
            position: 'top',
            message: 'No se puede enviar la revisión del chat.',
            actions: [{
              icon: 'close',
              round: true,
              color: 'white'
    ...
    async obterContato(key) {
      ...
      try {
        ...
        console.log('Error al recuperar el contacto: ' + err)
      }
    },
    ...
    async consultarTickets (paramsInit = {}) {
      ...
      try {
        ...
        this.$notificarErro('Algo salió mal', err)
        console.error(err)
      }
      ...
      if(data[0]?.status !== 'active'){
        ...
        if (data[0]?.asaas === 'enabled'){
          try{
            ...
            console.log('No se pueden mostrar las opciones de pago')
          }
        }
      }
    },
    ...
    async listarMotivos () {
      try {
        ...
        console.error('Motivos del error:', error);
      }
    },
    async handlePrivateMessageNotification(notification) {
      ...
      if (!isRecording && this.notificacaoAtivo === 'enabled') {
        ...
        console.log('La notificación de audio no se reproduce. Motivos:')
  ...
  async mounted() {
    ...
    if (storedColors) {
      ...
      console.warn('No hay colores almacenados en la localStorage');
    }
