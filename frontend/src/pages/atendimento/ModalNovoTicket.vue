    ...
    abrirAtendimentoExistente (contato, ticket) {
      if(this.isNotViewAssignedTickets === 'enabled' && this.userProfile === 'user'){
        this.$q.dialog({
          ...
          message: `${contato.name} Ya tienes un servicio en curso. Solo un administrador puede asignártelo..`,
          ok: {
      ...
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `${contato.name} Ya tienes un servicio en curso. (Chat: ${ticket.id} - Fila: ${
          ticket.queueId || 'Sin departamento.'
