    ...
    abrirAtendimentoExistente (contato, ticket) {
      if(this.isNotViewAssignedTickets === 'enabled' && this.userProfile === 'user'){
        this.$q.dialog({
          ...
          message: `${contato.name} Ya tienes un chat activo. Solo un administrador puede asignártelo.`,
          ok: {
      ...
      this.$q.dialog({
        title: this.$t('common.attention'),
        message: `${contato.name} Ya tienes un servicio activo (chat: ${ticket.id} - Fila: ${
          ticket.queueId || 'sem fila'
        }). ¿Quieres abrir este chat?`,
        cancel: {
