  ...
  methods: {
    ...
    formatarMensagemRespostaNfmWhatsapp(body) {
      ...
      try {
        ...
        console.error('Error al formatear la respuesta NFM:', error);
        return `➡️ ${body}`;
      }
    },
    formatarNotas(body) {
    ...
    formatarMensagemAnuncioExterno(body) {
      ...
      try {
        ...
        if (!body.includes('📱 Multimedia:') && !body.includes('🔗 Link:') && !body.includes('📱 App:')) {
          ...
          if (body.includes('[Multimedia]') || body.includes('[Thumbnail]') || body.includes('[Conversión]') || body.includes('[Mensaje de bienvenida]')) {
        ...
        if (infoMidia) {
          ...
          linhas.forEach(linha => {
            ...
            if (linha.startsWith('📱 Mídia:')) {
              const tipo = linha.replace('📱 Multimedia:', '').trim();
              ...
              html += `<div class="midia-card">
                        <div class="midia-icon">📱</div>
                        <div class="midia-info">
                          <div class="midia-label">App de origen</div>
        ...
        console.error('Error al formatear el mensaje de anuncio externo:', error);
        return this.formatarMensagemWhatsapp(body);
      }
    },
        ...
        if (infoAnuncio) {
          ...
          if (descricaoAnuncio) {
            html += `<div class="midia-card" style="display: flex; align-items: center; padding: 10px; margin: 8px 0; background: linear-gradient(135deg, #4ecdc4, #44a08d); border-radius: 8px; color: white;">
                      <div class="midia-icon" style="width: 32px; height: 32px; background: rgba(255,255,255,0.2); border-radius: 50%; display: flex; align-items: center; justify-content: center; margin-right: 12px; font-weight: bold; font-size: 14px;">📝</div>
                      <div class="midia-info" style="flex: 1;">
                        <div class="midia-label" style="font-size: 11px; opacity: 0.8; text-transform: uppercase; letter-spacing: 0.5px;">Descripción</div>
          ...
          if (linkMidia) {
            html += `<div class="midia-card" style="display: flex; align-items: center; padding: 10px; margin: 8px 0; background: linear-gradient(135deg, #ffecd2, #fcb69f); border-radius: 8px;">
                      ...
                      <div class="midia-info" style="flex: 1;">
                        <div class="midia-label" style="font-size: 11px; color: #666; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 2px;">Enlace de medios</div>
                        <div class="midia-value">
    ...
    formatarOrder(body) {
      try {
        ...
        let formattedBody = body
          ...
          .replace(/(💰.*?Total: )(\d+\.\d{2})( Moneda)/g, '<div style="background: linear-gradient(135deg, #28a745, #20c997); color: white; border-radius: 12px; padding: 15px; text-align: center; font-weight: bold; font-size: 18px; margin-top: 15px; box-shadow: 0 4px 8px rgba(40, 167, 69, 0.3);">$1<span style="background: rgba(255,255,255,0.2); padding: 4px 8px; border-radius: 8px; margin: 0 4px;">$2</span>$3</div>'); // Total destacado com preço em destaque

        // Adicionar estilos específicos para seções
        formattedBody = formattedBody
          .replace(/(📋.*?Detalles del pedido:)/g, '<div style="background: rgba(40, 167, 69, 0.1); border-left: 4px solid #28a745; padding: 12px; border-radius: 0 8px 8px 0; margin: 15px 0;"><strong style="color: #28a745;">$1</strong>')
          .replace(/(📦.*?Productos:)/g, '<div style="background: rgba(40, 167, 69, 0.1); border-left: 4px solid #28a745; padding: 12px; border-radius: 0 8px 8px 0; margin: 15px 0;"><strong style="color: #28a745;">$1</strong>');
        ...
        console.error('Error al formatear el mensaje del pedido:', error);
    ...
    formatarOrderMessage(body) {
      try {
        ...
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Detalhes do pedido</div>';
        html += '</div>';
        html += '</div>';
        ...
        console.error('Error de formato orderMessage:', error);
        // Fallback para formatação simples
    ...
    formatarProductMessage(body) {
      try {
        ...
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Detalhes do produto</div>';
        html += '</div>';
        html += '</div>';
        ...
        html += '<span style="color: #6c757d; font-size: 11px; font-weight: 500;">Producto en catálogo</span>';
        html += '</div>';
    ...
    formatarLocationRequest(body) {
      try {
        ...
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Haz clic para compartir</div>';
        html += '</div>';
        html += '</div>';
        ...
        html += '📍 Enviar ubicación';
        html += '</button>';
        html += '</div>';
        
        // Footer
        html += '<div style="margin-top: 20px; padding-top: 15px; border-top: 1px solid #dee2e6; text-align: center;">';
        html += '<span style="color: #6c757d; font-size: 11px; font-weight: 500;">Solicitar ubicación</span>';
        html += '</div>';
        ...
        return `<div style="background: #f8f9fa; border: 1px solid #007bff; border-radius: 8px; padding: 12px; margin: 8px 0;">
                  <div style="font-weight: bold; color: #495057; margin-bottom: 8px;">📍 Solicitar ubicación</div>
                  <div style="color: #666; font-size: 14px; white-space: pre-line;">${body}</div>
                </div>`;
      }
    },
    ...
    formatarCTAURL(body) {
      try {
        ...
        html += '<div style="font-size: 13px; color: #6c757d; font-weight: 500;">Haz clic para acceder al enlace</div>';
        html += '</div>';
        html += '</div>';
        ...
        console.error('Error de formato de la URL de llamada a la acción:', error);
        // Fallback para formatação simples
