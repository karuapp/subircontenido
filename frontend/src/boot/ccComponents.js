...
const iniciaisString = nomecompleto => {
  ...
  try {
    ...
    console.error('Error al procesar las iniciales:', error);
    return '';
  }
}
...
const setConfigsUsuario = ({ isDark }) => {
  ...
  if (usuario && usuario.userId) {
    UpdateConfiguracoesUsuarios(usuario.userId, data)
      .then(() => console.log('Configuración de usuario actualizada'))
      .catch(err => console.error('Error al actualizar la configuración de usuario', err))
