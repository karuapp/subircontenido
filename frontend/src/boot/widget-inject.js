...
(function() {
    ...
    <div id="channel-chat-container">
        <div id="channel-chat-header">
            ...
            <div style="display: flex; gap: 8px;">
                <button id="channel-chat-clear" title="Nueva sesión" style="background: none; border: none; color: white; cursor: pointer; font-size: 18px;"><i class="mdi mdi-reload" style="font-size:16px;"></i></button>
        ...
        <div id="channel-chat-input-area">
            ...
            <button id="channel-chat-attach" title="Adjuntar archivo" style="background: none; border: none; color: #8903ff; cursor: pointer; font-size: 20px; padding: 0 8px;"><i class="mdi mdi-paperclip"></i></button>
            <button id="channel-chat-send" title="Enviar mensaje" style="background: none; border: none; color: #8903ff; cursor: pointer; font-size: 20px; padding: 0 8px;"><i class="mdi mdi-send"></i></button>
        </div>
    </div>
    ...
    async function registerWebchat() {
        ...
        const tenantId = '1';
        const wabaId = '2011e4cc-259d-4514-bf62-92098d97d5df';
        const websocketToken = '577dafb1-38b0-4429-af4b-a308b8475010';
    ...
    function getAckIcon(ack) {
        ...
        if (ack === 3) return '<span style="color:#8903ff;">✔️✔️</span>';
        if (ack === -1) return '❌';
        return '';
    }
    ...
    function connectWebSocket() {
        ...
        function connect() {
            const wabaId = '2011e4cc-259d-4514-bf62-92098d97d5df';
            const websocketToken = '577dafb1-38b0-4429-af4b-a308b8475010';
            ws = new WebSocket(`wss://webchat2.watitan.com/wss?from=${webchatId}&token=${token}`);
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'pong') {
                        console.log('[WebChat] Pong recibido');
                    }
                } catch (error) {
                    console.error('[WebChat] Error al procesar el mensaje de WebSocket:', error);
                }
            };

            ws.onerror = (error) => {
                console.error('[WebChat] Error de conexión de WebSocket:', error);
            };

            ws.onclose = () => {
                console.log('[WebChat] Conexión de WebSocket cerrada');
